...
fn write_to_ring(ring_prod: &rb::Producer<f32>, samples: &[f32], stop_flag: &Arc<SharedState>) -> bool {
    ...
    while written < samples.len() {
        ...
        // When paused, the output stream is paused and not consuming samples.
        // Wait until unpaused or stopped, without counting towards write timeout.
        while !stop_flag.playing.load(Ordering::Relaxed) {
            if stop_flag.stop_flag.load(Ordering::Relaxed) {
                return false;
            }
