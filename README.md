...
**v3.3 新增：28 个端点的 HTTP REST API、Headless 无 GUI 服务端、浏览器 WebUI 测试界面。**
...
```bash
cd webui
MUSICLI_HTTP_PORT=<端口> node server.js
# 浏览器打开 http://127.0.0.1:3001
```

顶栏输入 MusiCLI 的 Host 和 Port，点击 Connect 即可操控。
...
**v3.3 adds: 28-endpoint HTTP REST API, headless server mode, browser WebUI test panel.**
...
```bash
cd webui
MUSICLI_HTTP_PORT=<port> node server.js
# Open http://127.0.0.1:3001 in browser
```

Enter the MusiCLI host and port in the top bar, click Connect.
